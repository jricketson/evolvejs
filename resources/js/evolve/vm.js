...
(function() {
  var add, alloc, copy, divide, dupTop, eq, execute, gte, ifDo, ifNotDo, incCounter, incReadPtr, incWritePtr, jmpB, jmpF, jmpReadPtrB, jmpReadPtrF, jmpWritePtrF, look, lt, move, mult, nop, pop, popM, push, pushCounter, pushCpuTime, pushM, pushMemSize, pushReadPtr, pushSpeciesHashcode, pushWritePtr, resetCounter, runThread, setSpeed, sleep, turnR,
  ...
  CORE.vm = {
    ...
    */
    ...
    execute: execute = function(thread) {
      var invalidinstructionLogtemplate, logtemplate, opcode, operand, operator;
  ...
  CORE.vm.instructionSet = {
    nop: nop = function(thread, operand) {
      return thread.executionPtr += 1;
    },
    add: add = function(thread, operand) {
      var a;
      a = thread.stack.pop();
      if (a !== undefined) {
    ...
    mult: mult = function(thread, operand) {
      var a;
      a = thread.stack.pop();
      if (a !== undefined) {
        thread.stack.push(a * operand);
    ...
    dupTop: dupTop = function(thread, operand) {
      var a;
      a = thread.stack.pop();
      if (a !== undefined) {
        thread.stack.push(a);
    ...
    push: push = function(thread, operand) {
      thread.stack.push(operand);
      return thread.executionPtr += 1;
    },
    pop: pop = function(thread, operand) {
      var i;
      i = 0;
      while (i < operand) {
        thread.stack.pop();
    ...
    pushM: pushM = function(thread, operand) {
    ...
    popM: popM = function(thread, operand) {
    ...
    incCounter: incCounter = function(thread, operand) {
    ...
    resetCounter: resetCounter = function(thread, operand) {
      thread.counter[operand] = 0;
      return thread.executionPtr += 1;
    },
    pushCounter: pushCounter = function(thread, operand) {
      if (thread.counter[operand] === undefined) {
        thread.stack.push(0);
      } else {
    ...
    pushMemSize: pushMemSize = function(thread, operand) {
      thread.stack.push(thread.process.memory.length);
      return thread.executionPtr += 1;
    },
    pushCpuTime: pushCpuTime = function(thread, operand) {
      thread.stack.push(thread.process.cputime);
      return thread.executionPtr += 1;
    },
    pushSpeciesHashcode: pushSpeciesHashcode = function(thread, operand) {
      thread.stack.push(thread.process.species.hashcode);
      return thread.executionPtr += 1;
    },
    pushReadPtr: pushReadPtr = function(thread, operand) {
      thread.stack.push(thread.readPtr);
      return thread.executionPtr += 1;
    },
    pushWritePtr: pushWritePtr = function(thread, operand) {
      thread.stack.push(thread.writePtr);
      return thread.executionPtr += 1;
    },
    jmpReadPtrB: jmpReadPtrB = function(thread, operand) {
      CORE.vm._jmpPtr(thread, thread.executionPtr, false, operand, "readPtr");
      return thread.executionPtr += 1;
    },
    jmpReadPtrF: jmpReadPtrF = function(thread, operand) {
      CORE.vm._jmpPtr(thread, thread.executionPtr, true, operand, "readPtr");
      return thread.executionPtr += 1;
    },
    jmpWritePtrF: jmpWritePtrF = function(thread, operand) {
      CORE.vm._jmpPtr(thread, thread.executionPtr, true, operand, "writePtr");
      return thread.executionPtr += 1;
    },
    jmpB: jmpB = function(thread, operand) {
      return CORE.vm._jmpPtr(thread, thread.executionPtr, false, operand, "executionPtr");
    },
    jmpF: jmpF = function(thread, operand) {
      return CORE.vm._jmpPtr(thread, thread.executionPtr, true, operand, "executionPtr");
    },
    incReadPtr: incReadPtr = function(thread) {
      thread.readPtr += 1;
      return thread.executionPtr += 1;
    },
    incWritePtr: incWritePtr = function(thread) {
      thread.writePtr += 1;
      return thread.executionPtr += 1;
    },
    copy: copy = function(thread) {
    ...
    ifDo: ifDo = function(thread, operand) {
      var a;
      a = thread.stack.pop();
      if (a) {
        return thread.executionPtr += 1;
    ...
    ifNotDo: ifNotDo = function(thread, operand) {
      var a;
      a = thread.stack.pop();
      if (a) {
        return CORE.vm._jmpPtr(thread, thread.executionPtr, true, operand, "executionPtr");
    ...
    runThread: runThread = function(thread) {
      var newThread;
    ...
    alloc: alloc = function(thread, operand) {
      var a, cost, finalLength, ii, success;
      a = thread.stack.pop();
      success = true;
      if (a !== undefined) {
    ...
    */
    ...
    move: move = function(thread) {
      var coords;
      coords = CORE.vm._calculateXYForward(thread.process.gridX, thread.process.gridY, thread.process.facing);
      CORE.environment.moveProcess(thread.process, coords[0], coords[1], coords[2]);
      return thread.executionPtr += 1;
    },
    sleep: sleep = function(thread, operand) {
      thread.sleepCycles = operand;
      return thread.executionPtr += 1;
    },
    setSpeed: setSpeed = function(thread, operand) {
