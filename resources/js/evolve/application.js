CORE.evolve = {
   ...
   _colours : [ "#000000", "#000033", "#000066", "#000099", "#0000CC", "#0000FF", "#003300",
   ...
   _sidebarCreatedCallback : function(gadget) {
      ...
      this._gridDisplay = $("div#gridDisplay").click(
            $.proxy(this._processClickedHandler, this));

      this._speciesList = $(".speciesList").bind("click",
            $.proxy(this._speciesClickedHandler, this));
      this._calculateMarkerSize();
      $(window).resize($.proxy(this._calculateMarkerSize, this));
      setInterval($.proxy(this.updateDisplay, this), this._timeDelay);
   },
   ...
   _processCreateHandler : function(e, process) {
      ...
      var divMarker = $(
            '<div class="process species{speciesId}" id="{processId}">&nbsp;</div>'.supplant( {
               speciesId : process.species.id,
               processId : process.id
            })).appendTo(this._gridDisplay).css( {
         top : this._markerHeight * process.gridY,
         left : (this._markerWidth * process.gridX),
         height : this._markerHeight - 1,
         width : this._markerWidth - 1,
   ...
   _speciesCreateHandler : function(e, species) {
      //$.debug(e, species);
      e.stopImmediatePropagation();
      var colour = this._colours.shift();
      var contrast = Contrast.match(colour, this._colours);
      var divMarker = $(this._speciesDivTemplate.supplant( {
      ...
      this._speciesDivStore[species.id] = {
         div : divMarker,
         species : species,
         colour : colour
      };
   ...
   _updateSpeciesDisplay : function() {
      for ( var speciesId in this._speciesDivStore) {
         if (this._speciesDivStore.hasOwnProperty(speciesId)) {
            this._updateSpeciesDiv(this._speciesDivStore[speciesId].div,
                  this._speciesDivStore[speciesId].species);
         }
      }
   },
   ...
   _speciesExtinctHandler : function(e, species) {
      ...
      delete this._speciesDivStore[species.id];
   },
   ...
   _processClickedHandler : function(e) {
      ...
      if (divClicked !== undefined) {
         this.setCurrentlyDisplayedProcess(this._processStore[divClicked.id]);
      }
   },
   ...
   _threadHtml : '<div class="pane collapsed thread{threadNumber}">' +
          '<div class="title">thread {threadNumber}</div>' + '<table class="content">' +
   ...
   _updateProcessDisplay : function updateProcessDisplay() {
      ...
      var process = this._currentlyDisplayedProcess;
      if (process !== null) {
   ...
   setColourForSpecies : function(species, colour) {
      var speciesEntry = this._speciesDivStore[species.id];
