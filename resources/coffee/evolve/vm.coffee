CORE.vm =
  ...
  _jmpPtr: (thread, start, dirForward, label, ptrName) ->
    ...
    if pos is -1
      pos = CORE.vm._searchArray(thread.process.memory, ((if dirForward then 0 else thread.process.memory.length - 1)), dirForward, label)

    if pos > -1
      thread[ptrName] = pos
    else
      #didn't find the jmp target, just advance one instruction
  ...
  execute: (thread) -> # executes a function on a thread
    ...
    if operator
      ...
      @_logOperation(thread, operator, operand)
    else
      @_logInvalidOperation(thread, opcode, operand)
...
CORE.vm.instructionSet =
  ...
  pushM: (thread, operand) -> # pushes operand from mem[operand] to stack
    if thread.shortTermMemory[operand]?
      thread.stack.push thread.shortTermMemory[operand]
    else
      thread.stack.push 0
    thread.executionPtr += 1

  popM: (thread, operand) -> # pops stack to operand at memPtr(+operand)
  ...
  resetCounter: (thread, operand) ->
    thread.counter[operand] = 0
    thread.executionPtr += 1

  pushCounter: (thread, operand) ->
    if thread.counter[operand]?
  ...
  copy: (thread) -> # copies memory from readPtr to writePtr
  ...
  runThread: (thread) -> # runs a separate thread in this process, the
  ...
  alloc: (thread, operand) ->
    ...
    if a isnt `undefined`
      ...
      if thread.process.cputime > cost
        ...
        for ii in [thread.process.memory.length...finalLength]
