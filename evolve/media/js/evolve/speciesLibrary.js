...
CORE.speciesLibrary = function() {
	...
	var colours = ["#000000", "#000033", "#000066", "#000099", "#0000CC",
	...
	// hashcode
	return {
		placeProcess : function placeProcess(process) {
			...
			if (!species) {
				...
				jQuery(document).trigger(
						CORE.environment.EVENT_SPECIES_CREATED, [species]);
			}
		...
		removeProcess : function(process) {
			...
			if (process.species.processes.length === 0) {
				...
				jQuery(document).trigger(
						CORE.environment.EVENT_SPECIES_EXTINCT,
						[process.species]);
			}
