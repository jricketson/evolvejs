CORE.assembler = function() {
	...
	return {
		/**
		 * takes an array of arrays [[operator, operand],...] and converts to
		 * compiled code in an internal representation
		 */
		...
		/**
		 * takes an array of operations in internal representation and converts
		 * to an array of arrays that is somewhat readable
		 */
		...
		makeDisplayableHtml : function makeDisplayableHtml(codeArray) {
			...
			for (var ii = 0; ii < humanReadable.length; ii += 1) {
				codeHtml += '["' + humanReadable[ii][0] + '", '
						+ humanReadable[ii][1] + '],<br/>';
			}
			return codeHtml;
		}
	};
