dojo.declare("CORE.Process", null, {
            ...
            setMemory : function(memory) {
				this.memory = memory;
				this.operands = []; // this is kept as a copy of the operands in the main
				// memory. It is purely used as a read optimisation for searching for operator patterns
			...
			/**
			 * steps the process through a single cycle, may be multiple
			 * instructions, each thread gets to execute
			 */
			step : function() {
			...
			/**
			 * keeps a copy of the operands in a separate list. This copy is
			 * used in vm.js - searchArray
			 */
			spliceMemory : function(position, elementCount, element) {
				this.memory.splice(position, elementCount, element);
				var op = "" + element[0];
				this.operands.splice(position, elementCount, (op.length == 1
								? '0' + op
								: op));
			},
			/**
			 * decrements the processes cputime, if the available cputime ever
			 * drops below 0, the process is killed
			 */
			...
			getHashCode : function() {
				...
				for (var ii = 0; ii < this.memory.length && ii < maxcheck; ii += 1) {
					if (this.memory[ii][0] != 0) {
						hash += (this.memory[ii][0] + 1 + this.memory[ii][1])
								* ii;
					} else {
						hash += 1;
					}
